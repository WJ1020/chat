...
public class AbsencesDao {
    ...
    public List<Absences> findByCourse(String course){
        String sql="SELECT id,student.sno,name,six,college,major,grade,course,date,state FROM absences LEFT JOIN student ON student.sno=absences.sno WHERE course=?";
        List<Absences> absences=this.jdbcTemplate.query(sql,new Object[]{course},new AbsencesMapper());
        return absences;
    }
    public List<Absences> findAll(){
        String sql="SELECT id,student.sno,name,six,college,major,grade,course,date,state FROM absences LEFT JOIN student ON student.sno=absences.sno";
    ...
    public static final class AbsencesMapper implements RowMapper<Absences>{
        ...
        public Absences mapRow(ResultSet resultSet, int i) throws SQLException {
            Student student=new Student(resultSet.getString("sno"),resultSet.getString("name"),resultSet.getInt("six"),resultSet.getString("college"),resultSet.getString("major"),resultSet.getString("grade"));
